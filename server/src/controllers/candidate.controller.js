...
export const handleAddCandidate = asyncHandler(async (req, res) => {
  ...
  if (election.status === "completed" || election.status === "cancelled") {
    throw new ApiError(
      400,
      "Cannot add candidates to a completed or cancelled election",
    );
  }
...
export const handleUpdateCandidate = asyncHandler(async (req, res) => {
  ...
  let finalPartySymbol =
    party_symbol !== undefined ? party_symbol : existing.party_symbol;
  ...
  if (req.files) {
    ...
    if (req.files["party_symbol_file"] && req.files["party_symbol_file"][0]) {
      ...
      if (
        existing.party_symbol &&
        existing.party_symbol.includes("cloudinary.com")
      ) {
        try {
