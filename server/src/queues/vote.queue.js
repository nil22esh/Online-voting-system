  ...
  async (job) => {
    ...
    try {
      ...
      try {
        ...
        logger.error(
          `Failed to broadcast socket update: ${socketError.message}`,
        );
      ...
      try {
        ...
        if (user && user.email && election) {
          sendVoteConfirmationEmail(user, election.title).catch((emailErr) =>
            logger.error(
              `Failed to send vote confirmation email: ${emailErr.message}`,
            ),
          );
        }
      } catch (infoError) {
        logger.error(
          `Failed to fetch info for confirmation email: ${infoError.message}`,
        );
      }
